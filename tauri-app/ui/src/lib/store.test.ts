 ...
 *
 * Note: formatMoney / pct / tickToDate are covered by format.test.ts.
 */
...
import { sim, ui, navigate, initRouting, exportMetricsCsv,
         beginLoad, endLoad, setError }                       from "./store.svelte";
...
describe("exportMetricsCsv", () => {
  ...
  it("CSV header row matches the field order of the first metricsRows entry", async () => {
    const row = {
      ...
      mean_productivity: 0.6, mean_income: 3000, mean_wealth: 0, state_capacity_score: 1.0, approval_q1: 0.5, approval_q2: 0.5, approval_q3: 0.5, approval_q4: 0.5, approval_q5: 0.5,
    };
    sim.metricsRows = [row];
    let capturedBlob: Blob | null = null;
    (URL.createObjectURL as ReturnType<typeof vi.fn>).mockImplementation((b: Blob) => {
  ...
  it("CSV data row values match the source TickRow", async () => {
    const row = {
      ...
      mean_productivity: 0.55, mean_income: 2500, mean_wealth: 0, state_capacity_score: 0.85, approval_q1: 0.5, approval_q2: 0.5, approval_q3: 0.5, approval_q4: 0.5, approval_q5: 0.5,
    };
    sim.metricsRows = [row];
    let capturedBlob: Blob | null = null;
    (URL.createObjectURL as ReturnType<typeof vi.fn>).mockImplementation((b: Blob) => {
  ...
  it("multi-row CSV: produces exactly 1 header + N data lines, values match per row", async () => {
    const makeRow = (tick: number, approval: number) => ({
      ...
      mean_health: 0.75, mean_productivity: 0.6, mean_income: 2800, mean_wealth: 0,
